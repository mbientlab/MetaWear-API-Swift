...
struct RawLogEntryParsingTests {
    ...
    @Test func parseAll_singleEntry() throws {
        // Header [0x0B, 0x07] + entry: id=0x05, tick=1(3LE), data=0x3412CDAB
        let notification = Data([0x0B, 0x07,  0x05, 0x01, 0x00, 0x00,  0xAB, 0xCD, 0x12, 0x34])
    ...
    @Test func parseAll_doubleEntry() throws {
        ...
        // Entry 1: id=0x00, tick=1, rawData=0x11223344
        notification += Data([0x00, 0x01, 0x00, 0x00, 0x44, 0x33, 0x22, 0x11])
        // Entry 2: id=0x01, tick=2, rawData=0x55667788
        notification += Data([0x01, 0x02, 0x00, 0x00, 0x88, 0x77, 0x66, 0x55])
        #expect(notification.count == 18)
    ...
    @Test func parseAll_epochMs_matchesTick() throws {
        let notification = Data([0x0B, 0x07,  0x00, 0x02, 0x00, 0x00,  0x00, 0x00, 0x00, 0x00])
        let entries = try RawLogEntry.parseAll(from: notification)
...
struct LogDataChunkTests {
    ...
    @Test func accel_chunkPackedBytes_areCorrect() {
        let chunks = MWAccelerometerBMI160(odr: .hz100, range: .g2).logDataChunks
        // Chunk 0: (offset=0 << 5) | (length=4 - 1) = 0x03
        #expect((chunks[0].offset << 5) | (chunks[0].length &- 1) == 0x03)
        // Chunk 1: (offset=4 << 5) | (length=2 - 1) = 0x81
        #expect((chunks[1].offset << 5) | (chunks[1].length &- 1) == 0x81)
    }
}
...
struct LogSampleDecodeTests {
    ...
    @Test func accel_reassembleAndDecode() throws {
        let sensor = MWAccelerometerBMI160(odr: .hz100, range: .g2)  // scale = 16384
        // Two log entries for the same sample at tick=1:
        ...
        let entries: [RawLogEntry] = [
            try RawLogEntry(entryBytes: Data([0x00, 0x01, 0x00, 0x00,  0x00, 0x40, 0x00, 0x00])),
            try RawLogEntry(entryBytes: Data([0x01, 0x01, 0x00, 0x00,  0x00, 0x20, 0x00, 0x00])),
        ]
